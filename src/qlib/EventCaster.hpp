...
public:
    ...
    {
        return m_listenerList.begin();
    }
    const_iterator end() const
    {
        return m_listenerList.end();
    }
    ...
    bool isRegistered(_EvCallBkType *pCB) const
    {
        ...
        for (; iter != eiter; ++iter)
            if (iter->second == pCB) return true;
        return false;
    }
    ...
    void fire(_EvntType &ev)
    {
        const_iterator iter = m_listenerList.begin();
        const_iterator eiter = m_listenerList.end();
        for (; iter != eiter; iter++) execute(ev, iter->second);
    }
    ...
    void replicaFire(_EvntType &ev)
    {
...

        {
            ...
            const_iterator iter = m_listenerList.begin();
            const_iterator eiter = m_listenerList.end();
            for (i = 0; iter != eiter; iter++, i++) cblist[i] = (iter->second);
            // unlock();
        }

        for (i = 0; i < nsize; ++i) execute(ev, cblist[i]);
    }
