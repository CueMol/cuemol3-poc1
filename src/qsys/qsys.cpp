...
void loadSysConfig(const LString &path)
{
    ...
    if (spos > 0) fdir = path.substr(0, spos);
    if (fdir.isEmpty()) fdir = ".";
...
bool init(const char *config)
{
    ...
    if (confpath.isEmpty()) return false;

    printf("qsys_regClasses OK\n");
    loadSysConfig(confpath);

    if (!RendererFactory::init()) return false;
    ...
    LOG_DPRINTLN("CueMol2 version %s (%s) build %s\n", pSceMgr->getVersion().c_str(),
                 pSceMgr->getVerArchName().c_str(), pSceMgr->getBuildID().c_str());
