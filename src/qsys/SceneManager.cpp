...
SceneManager::SceneManager()
{
    ...
    m_strVerInfo =
        LString::format("%d.%d.%d.%d", m_verInfo.major_version, m_verInfo.minor_version,
                        m_verInfo.revision, m_verInfo.build_no);
...
{
    if (uid == qlib::invalid_uid) return ScenePtr();

    data_t::const_iterator iter = m_data.find(uid);

    if (iter == m_data.end()) return ScenePtr();
    return iter->second;
}
...
ScenePtr SceneManager::getSceneByName(const LString &name) const
{
    ...
    for (; iter != eiter; ++iter) {
        if (name.equals(iter->second->getName())) return iter->second;
    }
...
void SceneManager::dump() const
{
    ...
    data_t::const_iterator eiter = m_data.end();
    for (; iter != eiter; ++iter) {
        MB_DPRINT("%p/%d (nref=%d): ", iter->second.get(), iter->second->getUID(),
...
qlib::LScrSp<qlib::LScrObjBase> SceneManager::getUIDObj(qlib::uid_t uid) const
{
    ...
    qlib::LScrObjBase *pObj =
        dynamic_cast<qlib::LScrObjBase *>(pMgr->getObjectByUID(uid));
