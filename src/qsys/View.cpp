...
View::~View()
{
    ...
    if (m_pMscr != NULL) delete m_pMscr;
...
void View::translateViewDrag(double dax, double day, double daz)
{
    ...
    if (qlib::abs<double>(daz) > F_EPS8) convZTrans(daz, dv2);
...
void View::getTrackRotQuat(double curX, double curY, double prevX, double prevY,
                           Vector4D &vaxis, double &rphi)
{
    ...
    if (!boost::math::isfinite(vlen) || qlib::isNear4(vlen, 0.0)) {
        vaxis = Vector4D();
        rphi = 0.0;
        return;
    }
...
void View::rotXY(double posx, double posy, double delx, double dely, double width,
                 double height)
{
    trackBallMove(double(posx) / width, double(posy) / height,
                  double(posx - delx) / width, double(posy - dely) / height);
    return;
}
...
bool View::mouseDragMove(InDevEvent &ev)
{
    ...
    if (xid == ViewInputConfig::VIEW_ROTX && yid == ViewInputConfig::VIEW_ROTY) {
        ...
        rotXY(ev.getX(), ev.getY(), ev.getDeltaX(), ev.getDeltaY(), width, height);
        forceRedraw();
        if (m_bRotMMS) m_pMscr->setType(MomentumScroll::MMS_ROTXY);
        return true;
    } else if (xid == ViewInputConfig::VIEW_ROTY && yid == ViewInputConfig::VIEW_ROTX) {
        double width = getWidth();
        double height = getHeight();
        rotXY(ev.getY(), ev.getX(), ev.getDeltaY(), ev.getDeltaX(), height, width);
        forceRedraw();
        return true;
    }
    ...
    if (xid == ViewInputConfig::VIEW_TRAX && yid == ViewInputConfig::VIEW_TRAY) {
        ...
        setViewCenterDrag(getViewCenter() - vec);
        forceRedraw();
        if (m_bTransMMS) m_pMscr->setType(MomentumScroll::MMS_TRANSXY);
        return true;
...
bool View::mouseDragEnd(InDevEvent &ev)
{
    ...
    if (m_pMscr->getType() != MomentumScroll::MMS_NONE) {
        ...
        if (peri > 0) qlib::EventManager::getInstance()->setTimer(this, peri);
    }
...
{
    if (m_listeners.remove(nid) == NULL) return false;
    return true;
}
...
void View::fireInDevEvent(InDevEvent &ev)
{
    ...
    ScrEventManager *pSEM = ScrEventManager::getInstance();
    bool ok = pSEM->fireEventScript(category, ScrEventManager::SEM_INDEV,
                                    ScrEventManager::SEM_OTHER, m_uid, ev);
...
DrawObjPtr View::getDrawObj(const LString &clsname)
{
    ...
    if (iter != m_drawObjTab.end()) return iter->second;
    ...
    if (pObj == NULL) {
        LString msg =
            LString::format("FatalERROR: Class %s is not DrawObj", clsname.c_str());
...
bool View::pushStyle(const LString &name)
{
    ...
    if (!m_pStyles->append(name)) return false;  // name is in already top style entry
...
bool View::removeStyleRegex(const LString &regex)
{
    ...
    if (!m_pStyles->removeByRe(regex)) return false;  // no match
...
{
    if (m_spViewFac == NULL) return NULL;
