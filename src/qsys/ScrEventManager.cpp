...
ScrEventManager::ScrEventManager()
{
    ...
    if (pLog != nullptr) {
        ...
        printf("LMsgLog add listener OK %d\n", m_nLogLsnID);
    } else {
        printf("LMsgLog is not initialized!!\n");
    }
}
...
{
    qlib::LMsgLog *pLog = qlib::LMsgLog::getInstance();
    if (pLog != NULL && m_nLogLsnID >= 0) pLog->removeListener(m_nLogLsnID);
    m_pCb = qlib::LSCBPtr();
    m_slot.clearAndDelete();
}

bool ScrEventManager::fireEventScript(const LString &aCatStr, int aTgtType,
                                      int aEvtType, qlib::uid_t aSrcID,
                                      qlib::LEvent &event)
{
    if (m_pCb.isnull()) return false;

    MB_DPRINTLN("fireEventScript category=%s tgttype=%d evttype=%d srcid=%d",
                aCatStr.c_str(), aTgtType, aEvtType, aSrcID);
    ...
    for (; iter != eiter; ++iter) {
        ...
        MB_DPRINTLN("Testing slot category=%s tgttype=%d evttype=%d srcid=%d",
                    iter->second->cat.c_str(), iter->second->nTgtType,
                    iter->second->nEvtType, iter->second->nSrcUID);
        ...
        if (iter->second->nTgtType < 0)
            bSrc = true;  // SEM_ANY
                 ...
                 iter->second->nTgtType)  //(aTgtType==iter->second->nTgtType)
            bSrc = true;
        if (!bSrc) {
            MB_DPRINTLN("NOT MATCHED");
            continue;
        ...
        MB_DPRINTLN("ScrEventManager::fireEventScript invoke for %p", m_pCb.get());

        // if (bCat&&bSrc&&bEvt&&bUID)
        // return iter->first;
        {
            const int nSlotID = iter->first;
            // MB_DPRINTLN("ScrEvent> fire event Slot=%d Cat=%s Tgt=%d Evt=%d", nSlotID,
            // aCatStr.c_str(), aTgtType, aEvtType);
...
bool ScrEventManager::fireEventImpl(QsysEvent &event)
{
    ...
    if (!event.getCategory(aCatStr, aTgtType, aEvtType)) return false;
...
int ScrEventManager::append(const LString &category, int nTgtType, int nEvtType,
                            int nSrcUID)
{
    ...
    MB_DPRINTLN("ScrEventManager append(%s, %d, %d, %d) OK -> %d",
                category.c_str(), nTgtType, nEvtType, nSrcUID, id);
    return id;
}
...
/// XXX: ???
int ScrEventManager::searchSlot(const LString &category, int nTgtType, int nEvtType,
                                qlib::uid_t nSrcUID)
{
    return -1;
}
...
{
    printf("ScrEventManager::logAppended called\n");
...
void ScrEventManager::addViewListener(qlib::uid_t nFilter, ViewEventListener *pL)
{
...
bool ScrEventManager::removeViewListener(ViewEventListener *pL)
{
    ...
    for (;;) {
        ...
        if (!bRemoved) return (nremv > 0) ? true : false;
    }
