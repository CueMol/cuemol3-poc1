...
LString SysConfig::get(const LString &key) const
{
    ...
    if (pNode == NULL) return LString();

    // pNode points to the target node
    const qlib::LVariant &lvar = pNode->getRawData();
    if (!lvar.isString()) return LString();  // type mismatch
...
void SysConfig::put(const LString &key, const LString &value)
{
    ...
    if (pNode == NULL) return;  // ignore errors
...
public:
    ...
    void startEntry(const Attributes &attr)
    {
        ...
        if (!strConst.isEmpty()) {
            if (strConst.equals("true"))
                ...
                LOG_DPRINTLN(
                    "SysConfig> Error: const attribute (%s) "
        ...
        if (strType.isEmpty() || strType.equals("string")) {
            ...
            if (strValue.toDouble(&n))
                ...
                LOG_DPRINTLN(
                    "SysConfig> Error: cannot convert value %s to real number.",
                    strValue.c_str());
                setError("Error.");
                return;
            }
    ...
    {
        if (name.equals("entry")) startEntry(attrs);
    }

    virtual void endElement(const LString &name)
    {
        if (name.equals("entry")) endEntry();
    }
};
...
void SysConfig::writeSection(Section *pSec)
{
    ...
    if (pSec->isConst()) ps.print(" const=\"true\"");
    ...
    if (pSec->hasData()) {
        ...
        if (lvar.isString()) {
            ...
            LOG_DPRINTLN(
                "SysConfig> warning: cannot serialize section"
...
static LString replaceDirective(const LString &path, const LString &drct,
                                const LString &data)
{
    if (!path.startsWith(drct)) return path;
...
LString SysConfig::convPathName(const LString &path) const
{
    ...
    if (ret.startsWith("%%CONFDIR%%")) {
        ...
        if (confdir.isEmpty()) {
            LOG_DPRINTLN(
                "SysConfig> Fatal error: cannot convert %%%%CONFDIR%%%% directive to "
        ...
        if (confdir.isEmpty()) {
            LOG_DPRINTLN(
                "SysConfig> Fatal error: cannot convert %%%%QSDIR%%%% directive to "
                "config file directory.");
            return ret;
        }
